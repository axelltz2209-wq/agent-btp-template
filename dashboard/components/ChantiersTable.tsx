...
export default function ChantiersTable({ chantiers }: ChantiersTableProps) {
  const getStatusBadge = (statut: string) => {
    const styles = { prevu: 'badge-prevu', en_cours: 'badge-encours', termine: 'badge-termine' }
    return styles[statut as keyof typeof styles] || styles.prevu
  }

  const getStatusLabel = (statut: string) => {
    const labels = { prevu: 'Prévu', en_cours: 'En cours', termine: 'Terminé' }
    ...
    <div className="card-btp overflow-hidden fade-in" style={{ animationDelay: '0.1s' }}>
      <div className="px-6 py-4" style={{ borderBottom: '1px solid #E2E8F0' }}>
        <div className="flex items-center gap-2.5">
          ...
          <h2 className="text-sm font-semibold text-slate-800">Chantiers à venir</h2>
          {chantiers.length > 0 && (
            <span className="ml-auto text-xs text-slate-400">{chantiers.length} chantiers</span>
          )}
        </div>
      </div>
                ...
                return (
                  <tr key={c.id}>
                    <td className="whitespace-nowrap">
                      <div className="text-sm font-medium text-slate-900">{c.client_nom}</div>
                    </td>
                    <td className="whitespace-nowrap">
                    ...
                    <td className="whitespace-nowrap">
                      <div className="text-sm text-slate-500">{getDaysText(daysUntil)}</div>
                    </td>
                    <td className="whitespace-nowrap">
                      <span className={`badge ${getStatusBadge(c.statut)}`}>{getStatusLabel(c.statut)}</span>
                    </td>
                  </tr>
                )
              })
